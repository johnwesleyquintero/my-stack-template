...
export function LogViewer({ level }: LogViewerProps) {
  ...
  const toggleLogExpansion = (logId: string) => {
    setExpandedLogs((current) => (current.includes(logId) ? current.filter((id) => id !== logId) : [...current, logId]))
  }
    ...
    <ScrollArea className="h-[600px] rounded-md border">
      <div className="divide-y">
        {filteredLogs.map((log) => (
          <div key={log.id} className="p-4 hover:bg-muted/50 cursor-pointer" onClick={() => toggleLogExpansion(log.id)}>
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">Source: {log.source}</p>
              </div>
            </div>
            {expandedLogs.includes(log.id) && log.details && (
              <div className="mt-2 rounded-md bg-muted p-2">
                <pre className="text-sm whitespace-pre-wrap">{log.details}</pre>
              </div>
            )}
          </div>
        ))}
